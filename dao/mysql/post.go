...
func CreatePost(post *models.Post) (err error) {
	...
	post_id, title, file_name, content, author_id, community_id)
	values(?,?,?,?,?,?)`
	_, err = db.Exec(sqlStr, post.PostID, post.Title, post.FileName,
...
func GetPostByID(pid int64) (post *models.Post, err error) {
	...
	sqlStr := `select post_id, title,file_name, content, author_id, community_id, status, create_time, update_time
	from post
	where post_id = ?`
...
func GetPostListByIDs(ids []string) (postList []*models.Post, err error) {
	sqlStr := `select post_id, title,file_name, content, author_id, community_id, create_time
	from post
	where post_id in (?)
...
func GetPostListByKeywords(p *models.ParamPostList) (posts []*models.Post, err error) {
	...
	sqlStr := `select post_id, title, content,file_name, author_id, community_id, create_time
	from post
	where title like ?
	or content like ?
	ORDER BY create_time
